...
def count_movies_by_genre(movies):
    genres = map(lambda movie: movie["genre"].lower(), movies)
...
def find_movie_by_title(movies, title):
    filtered_movies = filter(lambda movie: title in movie["title"], movies)
...
while True:
    ...
    if task == "1":
        ...
        if found_movie:
            print("\nInformasi Film:")
            show_movie_info(found_movie[0])  # Hanya menampilkan informasi film pertama jika ada beberapa film dengan judul yang sama
        else:
